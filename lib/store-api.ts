...
// Contest operations
export async function createContest(contest: Omit<Contest, 'id' | 'createdAt' | 'joinCode'>): Promise<Contest> {
...
export async function getParticipantByUserId(userId: string, contestId: string): Promise<Participant | undefined> {
  try {
    return await apiCall<Participant>(`/participants/${userId}/${contestId}`);
  } catch (error) {
    return undefined;
  }
}
